      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="w-full lg:w-3/5 bg-white rounded-3xl p-4 border border-slate-200 shadow-sm overflow-hidden">
          <svg viewBox="0 0 600 820" className="w-full h-auto max-h-[70vh]">
            ...
            <rect
              ...
              width="280"
              height="280"
              fill={selectedZone === 'zone_cyclorama' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-colors hover:fill-emerald-50"
              onClick={() => setSelectedZone('zone_cyclorama')}
            />
            <text
              x="150"
              y="150"
              textAnchor="middle"
              className="text-sm font-bold pointer-events-none fill-slate-700"
            >
              Cyclorama
            </text>
            ...
            <rect
              ...
              width="85"
              height="119"
              fill={selectedZone === 'zone_seating_setup' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_seating_setup')}
            />
            {/* Pantry & Equipment */}
            <rect
              x="10"
              ...
              width="102"
              height="76"
              fill={selectedZone === 'zone_pantry' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_pantry')}
            />
            <rect
              x="117"
              y="419"
              width="102"
              height="76"
              fill={selectedZone === 'zone_equipment' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
            ...
            <rect
              ...
              width="175"
              height="60"
              fill={selectedZone === 'zone_abstract_wall' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_abstract_wall')}
            />
            <rect
              x="475"
              y="10"
              width="115"
              height="119"
              fill={selectedZone === 'zone_makeup' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_makeup')}
            />
            <rect
              x="505"
              y="134"
              width="85"
              height="85"
              fill={selectedZone === 'zone_changing_room' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_changing_room')}
            />
            <rect
              x="505"
              y="224"
              width="85"
              height="204"
              fill={selectedZone === 'zone_royal_setup' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_royal_setup')}
            />
            <rect
              x="505"
              y="433"
              width="85"
              height="170"
              fill={selectedZone === 'zone_orange_niche_wall' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_orange_niche_wall')}
            />
            ...
            <rect
              ...
              width="170"
              height="34"
              fill={selectedZone === 'zone_staircase' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_staircase')}
            />
            <rect
              x="420"
              y="647"
              width="170"
              height="153"
              fill={selectedZone === 'zone_office' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
              onClick={() => setSelectedZone('zone_office')}
            />
            <rect
              x="223"
              y="509"
              width="85"
              height="153"
              fill={selectedZone === 'zone_waiting' ? '#10B981' : '#F8F9FA'}
              stroke="#1A1A1A"
              strokeWidth="2"
              className="cursor-pointer transition-all hover:fill-emerald-50 hover:stroke-emerald-500"
            ...
            {[1, 2, 3, 4, 5, 6].map((num, i) => (
              <g
                ...
                onClick={() => setSelectedZone(`zone_floating_${num}`)}
              >
                <circle
                  cx={num <= 3 ? 330 : 400}
                  cy={220 + (i % 3) * 50}
        ...
        <div className="w-full lg:w-2/5 space-y-4 sticky top-4">
            ...
            {selectedZone ? (
              <div className="animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="p-6">
                  ...
                  <p className="text-slate-400 mt-2 italic">
                    Dimensions:{' '}
                    <span className="text-white not-italic">{activeZone?.dimensions}</span>
                  </p>
              ...
              <div className="h-[400px] flex flex-col items-center justify-center p-8 text-center space-y-4">
                ...
                <p className="text-slate-400 text-sm">
                  Click any area on the studio map to view live setup photos and booking options.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-3xl hidden lg:block shadow-sm">
            <h4 className="text-sm font-bold text-slate-800 mb-4 uppercase tracking-tighter">
              Studio Overview
            </h4>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[10px]">
              {zones.slice(0, 10).map((z) => (
                ...
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${selectedZone === z.id ? 'bg-emerald-500 scale-125' : 'bg-slate-300'}`}
                  ></div>
                  {z.name}
                </div>
              ))}
