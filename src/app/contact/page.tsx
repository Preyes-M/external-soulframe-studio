...
export default function Contact() {
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      icon: 'EnvelopeIcon',
      title: 'Email',
      value: 'hello@soulframestudio.com',
      link: 'mailto:hello@soulframestudio.com',
            ...
            {contactInfo.map((info, index) => (
              ...
              >
                <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center mb-4 group-hover:bg-primary group-hover:text-primary-foreground transition-all">
                  <Icon name={info.icon as any} size={24} className="text-primary group-hover:text-primary-foreground" />
                </div>
            ...
            <div className="reveal delay-200 space-y-8">
              ...
              <div className="glass-panel rounded-3xl p-6">
                ...
                <div className="space-y-2">
                  {hours.map((hour) => (
                    <div key={hour.id} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                      <span className="text-sm text-muted-foreground">{hour.day}</span>
                      <span className={`text-sm font-medium ${hour.time === 'Closed' ? 'text-error' : 'text-foreground'}`}>
                        {hour.time}
                      </span>
                    </div>
                  ))}
